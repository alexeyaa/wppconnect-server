...
export async function approveGroupMembershipRequest(req: Request, res: Response) {
  /**
     #swagger.tags = ["Group"]
     #swagger.autoBody=false
     #swagger.security = [{
  ...
  try {
    const arrayGroups: any = [];
    for (const group of groupToArray(groupId)) {
      await req.client.approveGroupMembershipRequest(group, contactToArray(phone));
      arrayGroups.push(group);
    }
...
export async function rejectGroupMembershipRequest(req: Request, res: Response) {
  /**
     #swagger.tags = ["Group"]
     #swagger.autoBody=false
     #swagger.security = [{
  ...
  try {
    const arrayGroups: any = [];
    for (const group of groupToArray(groupId)) {
      await req.client.rejectGroupMembershipRequest(group, contactToArray(phone));
      arrayGroups.push(group);
    }
...
export async function getGroupMembershipRequests(req: Request, res: Response) {
  /**
     #swagger.tags = ["Group"]
     #swagger.autoBody=false
     #swagger.security = [{
  ...
  try {
    ...
    for (const group of groupToArray(groupId)) {
      response = await req.client.getGroupMembershipRequests(group);
      arrayGroups.push(response);
    }
